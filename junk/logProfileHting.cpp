...
// [[Rcpp::export]]
double logProfileCppHTing(const Eigen::VectorXd theta, const Eigen::MatrixXd DTR, 
  ...
  double logUdet = U.diagonal().array().log().sum(); // = 1/2log(Sigma)
  return(N*quadForm/2 + logUdet);
}
